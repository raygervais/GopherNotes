...
func QueryCommandStatement(database *sql.DB, command string, arguments ...interface{}) (*sql.Rows, error) {
	return database.Query(command, arguments...)
}
...
// SearchNotesByDate -
func SearchNotesByDate(database *sql.DB, date string) *sql.Rows {
	return DatabaseRowsErrorHandler(
		QueryCommandStatement(
			database, "SELECT id, entry, date from notes WHERE date LIKE  '%' || ? || '%' ", date))
}

func SearchNotesByEntry(database *sql.DB, entry string) *sql.Rows {
	return DatabaseRowsErrorHandler(
		QueryCommandStatement(
			database, "SELECT id, entry, date from notes WHERE entry LIKE  '%' || ? || '%' ", entry))
}
